...
class Preview extends RelativeLayout implements SurfaceHolder.Callback {
    ...
    public void switchCamera(Camera camera, int cameraId) {
        ...
        if (mSupportedPreviewSizes != null) {
            Log.d(TAG, "switchCamera call getOptimalPreviewSize()");
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (mSupportedPreviewSizes != null) {
            Log.i(TAG, "onMeasure getOptimalPreviewSize");
            mPreviewSize = getOptimalPreviewSize(mSupportedPreviewSizes, viewWidth, viewHeight);
...
//        }
        ...
        for (int i = 0; i < sizes.size(); i++) {
            if ((int) sizes.get(i).width * h == (int) sizes.get(i).height * w ){
        ...
        Log.d(TAG, "getOptimalPreviewSize: w: " + optimalSize.width + " h: " + optimalSize.height + "/viewWidth x viewHeight: " + w + " x " + h);
        return optimalSize;
    }
    ...
    public void surfaceChanged(SurfaceHolder holder, int format, int w, int h) {
        if (mCamera != null) {
            ...
            mSupportedPreviewSizes = mCamera.getParameters().getSupportedPreviewSizes();
            if (mSupportedPreviewSizes != null) {
                Log.d(TAG, "surfaceChanged call getOptimalPreviewSize()");
